...
import sampleFixture from "../../tests/fixtures/smartrecruiters-sample.json";
...
async function runDemonstration() {
  ...
  const demoSourceIdentifier = "demo-smartrecruiters-inc";

  console.log("================================================================================");
  console.log("🚀 SMARTRECRUITERS JOB SOURCE ADAPTER DEMONSTRATION");
  console.log("================================================================================\n");

  // Step 0: Ensure clean test source in database
  await supabase.from("canonical_jobs").delete().eq("source", "smartrecruiters").eq("source_id", demoSourceId);
  ...
  const demoSourceRecord: JobSourceRecord = {
    ...
    source_name: "SmartRecruiters Inc (Demo)",
  ...
  const adapter = getAdapterForSource("smartrecruiters");
  ...
  adapter.fetchJobs = async () => sampleFixture.content;
  ...
  console.log(`• Jobs Unchanged:   ${syncResult2.stats.jobsUnchanged} (Expected ${sampleFixture.content.length})`);
  ...
  const updatedJobs = [
    {
      ...sampleFixture.content[0],
      name: "Principal Information Security & AI Governance Engineer",
      compensation: {
        min: 210000,
        max: 270000,
        currency: "USD",
  ...
  // ---------------------------------------------------------------------------
  console.log("\n--------------------------------------------------------------------------------");
  console.log("▶ SCENARIO 4: Failed-Request Scenario (Upstream HTTP 500 / Network Error)");
  console.log("--------------------------------------------------------------------------------");

  adapter.fetchJobs = async () => {
    throw new Error("HTTP 500 Internal Server Error: SmartRecruiters Posting API temporarily unavailable");
  };
